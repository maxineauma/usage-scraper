...
const parser = new ArgumentParser({
  description: "Smogon usage statistics scraping tool.",
});
parser.add_argument("-d", "--date", {
  help: "Data in YYYY-MM format, example: 2022-01. Required.",
  required: true,
});
parser.add_argument("-g", "--gen", {
  help: "Pokemon generation number, example: 6. Required.",
  required: true,
});
parser.add_argument("-t", "--tier", {
  help: "Tier name, example: OU, National Dex. Required.",
  required: true,
});
parser.add_argument("-c", "--cutline", {
  help: "Usage stats cutline. Must be: 0, 1500, 1630, 1760. Default: 0.",
  default: 0,
});
...
(async () => {
  ...
  const _stat = await axios.get(
    "https://www.smogon.com/stats/" + _date + "/moveset/" + _scrape + ".txt"
  );
  const _rank = await axios.get(
    "https://www.smogon.com/stats/" + _date + "/" + _scrape + ".txt"
  );
  ...
  for (let x = 0; x < _stat_data.length; x++) {
    ...
    let pokemonName = regional_form_name(
      String(pokemon[0]).match(_name_reg)[0]
    );
    ...
    for (let x = 1; x < pokemAbil.length - 1; x++)
      pokemAbils[String(pokemAbil[x].match(_name_percent_reg)[1])] = parseFloat(
        pokemAbil[x].match(_name_percent_reg)[3]
      );
    ...
    for (let x = 1; x < pokemItem.length - 1; x++)
      pokemItems[String(pokemItem[x].match(_name_percent_reg)[1])] = parseFloat(
        pokemItem[x].match(_name_percent_reg)[3]
      );
    ...
    for (let x = 1; x < pokemSpread.length - 1; x++)
      pokemSpreads[String(pokemSpread[x].match(spread_reg)[1])] = parseFloat(
        pokemSpread[x].match(spread_reg)[3]
      );
    ...
    for (let x = 1; x < pokemMove.length - 1; x++)
      pokemMoves[String(pokemMove[x].match(_name_percent_reg)[1])] = parseFloat(
        pokemMove[x].match(_name_percent_reg)[3]
      );
    ...
    for (let x = 1; x < pokemTeam.length - 1; x++) {
      try {
        pokemTeams[
          regional_form_name(String(pokemTeam[x].match(_name_percent_reg)[1]))
        ] = parseFloat(pokemTeam[x].match(_name_percent_reg)[3]);
      } catch (e) {
        continue;
      }
    }
    ...
    for (let x = 1; x < pokemCC.length; x++) {
      ...
      try {
        pokemCCs[
          regional_form_name(String(pokemCC[x - 1].match(_name_reg)[1]))
        ] = String(pokemCC[x].match(stat_reg)[1]);
      } catch (e) {
        continue;
      }
    }
